...
class TestMemorySyncCalls:
    ...
    def test_store(
        ...
        entry = sync_client.memory.store(key="ctx", value="important data", tags=["s1"])
    ...
    def test_store_without_tags(
        ...
        mock_api.post("/v1/memory").mock(side_effect=capture)
        entry = sync_client.memory.store(key="k", value="v")
    ...
    def test_store_sends_correct_body(
        ...
        mock_api.post("/v1/memory").mock(side_effect=capture)
        sync_client.memory.store(key="k", value="v", tags=["t1", "t2"])
    ...
    def test_get(
        ...
        mock_api.get("/v1/memory/my-key").mock(
            return_value=httpx.Response(
                200,
                json={"key": "my-key", "value": "stored value", "tags": ["session-1"]},
            )
        )
        entry = sync_client.memory.get("my-key")
    ...
    def test_recall_with_tags(
        ...
        entries = sync_client.memory.recall(tags=["tag-a"])
    ...
    def test_recall_with_prefix(
        ...
        mock_api.post("/v1/memory/recall").mock(side_effect=capture)
        entries = sync_client.memory.recall(prefix="ctx:", limit=50)
        assert captured["body"]["prefix"] == "ctx:"
    ...
    def test_recall_empty_result(
        ...
        entries = sync_client.memory.recall()
        assert entries == []

    def test_delete(
        self, sync_client: SigmaShake, mock_api: respx.Router
    ) -> None:
        mock_api.delete("/v1/memory/my-key").mock(
            return_value=httpx.Response(200, json={})
        )
        # delete returns None; just verify no exception
        sync_client.memory.delete("my-key")
